function [I,E,theta,W,Loss,rho] = CoachLowRankSparseOptimize_colsparse2_byADMM(R,I_original,para)
...
%     if iter == 1
%         W_new =  OptimizeW_ByST(R,E_new,I,Y_1,beta,rho);
%     else
%         W_new =  OptimizeW_ByST(R,E_new,I_new,Y_1,beta,rho);
%     end
...
%     E_new =  OptimizeE_ByST(R,I,Y_1,lambda,rho);
    ...
    I_new = OptimizeI(R,E_new,W_new,theta_new,Y_1,Y_2,rho);
    I_new(idx_fixed) = 1.0;
    

    
    Y_1_new = Y_1 + rho*(R - R.*I_new - E_new -W_new );
...
%     loss(iter) = rank(theta_new)+ lambda*norm(E_new,1);
    constraint1(iter) = norm(R - R.*I_new - E_new -W_new,'fro');
...
function W_new = OptimizeW_ByST(R,E,I,Y_1,beta,rho)
alpha = beta*1.0/(rho);
% x =(E - Y_3*1.0/rho);
x = (R -E - R.*I + Y_1*1.0/rho);
...
%ʹ�ö��κ�������ֵ��ʽ������Ż�����I
function I_new = OptimizeI(R,E,W,theta,Y_1,Y_2,rho)
% step1 ʹ�ù�ʽ�������ֵ��
para_a = rho * R;
para_b = rho.*E - rho.*theta - rho.*R + rho.*W + Y_1 + Y_2;
I_new  = zeros(size(R));
